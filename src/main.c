...
static void output_navidoc_preamble(struct parser *p) {
	fprintf(p->output, ".\\\" Generated by navidoc " VERSION "\n");
...
int main(int argc, char **argv) {
	if (argc == 2 && strcmp(argv[1], "-v") == 0) {
		printf("navidoc " VERSION "\n");
		return 0;
	} else if (argc > 1) {
		fprintf(stderr, "Usage: navidoc < input.nvd > output.roff\n");
		return 1;
	}
	struct parser p = {
		.input = stdin,
